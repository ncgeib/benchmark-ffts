...
print('Mean Error Complex FFT (Non-Primes)')
...
for k in sorted(results.keys()):
    ...
    f2 = ~(f | fp)
    lc1, = ax1.plot(n[f], v['complex_l2'][f], **kwargs)
    lc2, = ax2.plot(n[f2], v['complex_l2'][f2], **kwargs)
    lc3, = ax3.plot(n[fp], v['complex_linf'][fp], **kwargs)
    lc4, = ax4.plot(v['identity_sizes'], v['identity'], **kwargs)
    i += 1

    # ignore prime sizes as the error grows strongly with N
    print(k.ljust(20), np.mean(v['complex_l2'][~fp]))
...
if save:
    fig.savefig('result_plots/accuracy_complex.png', dpi=300)
else:
    plt.show()
